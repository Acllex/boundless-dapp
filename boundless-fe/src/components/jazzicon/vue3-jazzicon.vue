<script setup lang="ts">
import MersenneTwister from 'mersenne-twister'
import addressToNumber from './addressToNumber'
import { colors } from './colors'
import paperGen from './paper'
import { onMounted, ref, watch } from 'vue'

const props = defineProps({
  diameter: {
    type: Number,
    required: true,
    default: 100
  },
  address: {
    type: String,
    required: true
  }
})
const svgns = 'http://www.w3.org/2000/svg'
const shapeCount = 4

const generator = ref() as any
const jazzicon = ref()
watch(
  () => props.address,
  (val) => {
    const el = generateIdenticon(props.diameter, addressToNumber(val))
    jazzicon.value.replaceChild(el, jazzicon.value.firstChild)
  }
)
onMounted(() => {
  const el = generateIdenticon(props.diameter, addressToNumber(props.address))
  jazzicon.value.appendChild(el)
})

function generateIdenticon(diameter: number, seed: number) {
  generator.value = new MersenneTwister(seed)
  const remainingColors = hueShift(colors.slice())

  const elements = paperGen(diameter, genColor(remainingColors))
  const container = elements.container

  const svg = document.createElementNS(svgns, 'svg')
  svg.setAttributeNS(null, 'x', '0')
  svg.setAttributeNS(null, 'y', '0')
  svg.setAttributeNS(null, 'width', diameter.toString())
  svg.setAttributeNS(null, 'height', diameter.toString())

  container.appendChild(svg)

  for (let i = 0; i < shapeCount - 1; i++) {
    genShape(remainingColors, diameter, i, shapeCount - 1, svg)
  }

  return container
}

function genShape(remainingColors: string[], diameter: number, i: number, total: number, svg: any) {
  const center = diameter / 2

  const shape = document.createElementNS(svgns, 'rect')
  shape.setAttributeNS(null, 'x', '0')
  shape.setAttributeNS(null, 'y', '0')
  shape.setAttributeNS(null, 'width', diameter.toString())
  shape.setAttributeNS(null, 'height', diameter.toString())

  const firstRot = generator.value.random()
  const angle = Math.PI * 2 * firstRot
  const velocity = (diameter / total) * generator.value.random() + (i * diameter) / total

  const tx = Math.cos(angle) * velocity
  const ty = Math.sin(angle) * velocity

  const translate = 'translate(' + tx + ' ' + ty + ')'

  // Third random is a shape rotation on top of all of that.
  const secondRot = generator.value.random()
  const rot = firstRot * 360 + secondRot * 180
  const rotate = 'rotate(' + rot.toFixed(1) + ' ' + center + ' ' + center + ')'
  const transform = translate + ' ' + rotate
  shape.setAttributeNS(null, 'transform', transform)
  const fill = genColor(remainingColors)
  shape.setAttributeNS(null, 'fill', fill)

  svg.appendChild(shape)
}

function genColor(colors: string[]) {
  //   var rand = generator.value.random()
  const idx = Math.floor(colors.length * generator.value.random())
  const color = colors.splice(idx, 1)[0]
  return color
}

const wobble = 30
function hueShift(colors: string[]) {
  const amount: number = generator.value.random() * 30 - wobble / 2
  const rotate = (hex: string) => colorRotate(hex, amount)
  return colors.map(rotate)
}

function colorRotate(hex: string, degrees: number) {
  const hsl = hexToHSL(hex)
  let hue: number = hsl.h
  hue = (hue + degrees) % 360
  hue = hue < 0 ? 360 + hue : hue
  hsl.h = hue

  return HSLToHex(hsl)
}

function hexToHSL(hex: string) {
  // Convert hex to RGB first
  let r = parseInt('0x' + hex[1] + hex[2])
  let g = parseInt('0x' + hex[3] + hex[4])
  let b = parseInt('0x' + hex[5] + hex[6])

  // Then to HSL
  r /= 255
  g /= 255
  b /= 255
  let cmin = Math.min(r, g, b),
    cmax = Math.max(r, g, b),
    delta = cmax - cmin,
    h = 0,
    s = 0,
    l = 0

  if (delta == 0) h = 0
  else if (cmax == r) h = ((g - b) / delta) % 6
  else if (cmax == g) h = (b - r) / delta + 2
  else h = (r - g) / delta + 4

  h = Math.round(h * 60)

  if (h < 0) h += 360

  l = (cmax + cmin) / 2
  s = delta == 0 ? 0 : delta / (1 - Math.abs(2 * l - 1))
  s = +(s * 100).toFixed(1)
  l = +(l * 100).toFixed(1)

  return { h, s, l }
}

function HSLToHex(hsl: any) {
  let { h, s, l } = hsl
  s /= 100
  l /= 100

  let c = (1 - Math.abs(2 * l - 1)) * s,
    x = c * (1 - Math.abs(((h / 60) % 2) - 1)),
    m = l - c / 2,
    r = 0,
    g = 0,
    b = 0

  if (0 <= h && h < 60) {
    r = c
    g = x
    b = 0
  } else if (60 <= h && h < 120) {
    r = x
    g = c
    b = 0
  } else if (120 <= h && h < 180) {
    r = 0
    g = c
    b = x
  } else if (180 <= h && h < 240) {
    r = 0
    g = x
    b = c
  } else if (240 <= h && h < 300) {
    r = x
    g = 0
    b = c
  } else if (300 <= h && h < 360) {
    r = c
    g = 0
    b = x
  }
  // Having obtained RGB, convert channels to hex
  let rStr = Math.round((r + m) * 255).toString(16)
  let gStr = Math.round((g + m) * 255).toString(16)
  let bStr = Math.round((b + m) * 255).toString(16)

  // Prepend 0s, if necessary
  if (rStr.length == 1) rStr = '0' + r
  if (gStr.length == 1) gStr = '0' + g
  if (bStr.length == 1) bStr = '0' + b
  return '#' + rStr + gStr + bStr
}
</script>
<template>
  <div ref="jazzicon" />
</template>
